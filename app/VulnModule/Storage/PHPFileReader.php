...
class PHPFileReader implements IReader
{
    ...
    {
        $type = in_array($config['type'] ?? null, Context::getTypes()) ? $config['type'] : Context::TYPE_STANDARD;
        $technology = in_array($config['technology'] ?? null, Context::getTechnologies()) ? $config['technology'] : Context::TECH_GENERIC;
        ...
        if (is_array($config['fields'] ?? null)) {
            ...
            foreach ($fields as $fieldData ) {
                ...
                $source = ($fieldData['source'] ?? null) ?: FieldDescriptor::SOURCE_ANY;
        ...
        if (is_array($config['children'] ?? null)) {
        ...
        if (is_array($config['vulnerabilities'] ?? null)) {
    ...
    protected function buildVulnerabilityElementFromArray($data, $conditional = false)
    {
        ...
        if ($conditional) {
            ...
            if (is_array($data['conditions'] ?? null)) {
