...
<div class="row">
                    ...
                    <form action="/search" name="filter-block">
                        <ul class="nav nav-list">
                            ...
                            @foreach($filterFabric->getFilter('Brand')->getVariants() as $id => $name)
                                @php $isChecked = $brand == $id ? 'active' : ''; @endphp
                                <li class="{!! $isChecked !!}">
                            ...
                            @foreach($filterFabric->getFilter('Price')->getVariants() as $id => $name)
                                @php $isChecked = $price == $id ? 'active' : ''; $elemId = 'price-' . $id; @endphp
                                <li class="{!! $isChecked !!}">
                            ...
                            @foreach($filterFabric->getFilter('Quality')->getVariants() as $id => $name)
                                @php $isChecked = $quality == $id ? 'active' : ''; $elemId = 'quality-' . $id; @endphp
                                <li class="{!! $isChecked !!}">
    ...
    <div class="col-xs-12 col-sm-9">
        ...
        <!-- END CONTENT ITEM -->
        @php $hasPagerItems = count($currentItems) > 0; @endphp
        @if($hasPagerItems)
            <div class="row product-list-inline-small">
                @foreach($currentItems as $item)
                    <div class="col-sm-6 col-md-3">
                        <div class="thumbnail">
                            <a href="/product/{!! $item->productID !!}"><img src="/products_pictures/{!! $item->picture ?: $item->big_picture !!}" alt="">
                            </a>
                            <div class="caption">
                                <a href="/product/{!! $item->productID !!}" title="{!! $item->name !!}">
                                    {!! mb_substr($item->name, 0, 30, 'utf-8') !!}
                                </a>
                                <p>
                                    <small title="{!! $item->description !!}">{!! mb_substr($item->description, 0, 80, 'utf-8') !!}</small>
