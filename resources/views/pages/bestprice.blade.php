...
<div class="container">
    ...
    <div class="section">
        <form role="form" method="post" action="/bestprice" id="bestpriceForm">
            ...
            {!! $controller->renderTokenField('bestprice') !!}
...
<script>
    ...
    $(function() {
        ...
        $('#bestpriceForm').bootstrapValidator({
            ...
            l.start();
            $.ajax({
                url: '/bestprice', type: "POST", dataType: "json", data: $("#bestpriceForm").serialize(),
                success: function(data) {
                    alertBlock.empty().append('Thank you for your question. We will contact you as soon.').show();
                    if (data.length) { ko.applyBindings(model); model.data(data); $('#accordion1').css('display', 'block'); }
                    else { alertBlock.empty().append('There is some error happened during processing your request.').show(); }
                },
                fail: function() { alertBlock.empty().append('There is some error happened during processing your request.').show(); }
