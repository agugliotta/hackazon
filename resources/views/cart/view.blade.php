...
<script>
    function update_qty(productId, qty) {
        ...
        $.ajax({
            ...
            success: function(data){
                if (qty <= 0) { $("#tr_item_" + productId).remove(); }
    ...
    $(function () {
        ...
        $("#empty_cart").click(function(){
            ...
            l.ladda('start');
            $.ajax({
                url:'/cart/empty', type:"POST", dataType:"json",
                success: function(data){
                    $(".tr_items").remove();
                    $("#items_qty").html(0);
                    $("#total_price").html(0);
                    ...
                    l.ladda('stop');
                    setTimeout(function () { $(".alert-info").alert("close"); location.reload(); }, 3000);
                },
                fail: function() { alert("error"); l.ladda('stop'); el.removeAttr('disabled'); }
            });
        });
        $(".close").click(function(){ $(".alert-info").alert('close'); });
        ...
        $("#step1_next").click(function(ev) {
            if (cardBlock.is(':visible')) {
                ...
                if (!validator.isValid()) { ev.preventDefault(); return; }
            }
            var el = $(this), l = el.ladda();
            el.attr('disabled', 'disabled');
            l.ladda('start');
    ...
    function successCouponCallback(result) { location.reload(); }
    function successCouponUnsetCallback(result) { location.reload(); }
    function invalidCouponCallback(error) {
        var el = $('#couponField').tooltip('destroy').tooltip({ title: 'Wrong coupon', delay: 3000 }).tooltip('show');
        setTimeout(function () { el.tooltip('hide'); }, 2000);
    }
    function getCouponWidget() { return getFlashMovie('coupon_as'); }
</script>
            ...
            <div class="col-xs-12">
                <table class="table table-order">
                    ...
                    <tfoot>
                        <tr class="info">
                            <td class="text-right" colspan="3">
                                <form class="form-horizontal" id="methods">
                                    {!! $controller->renderTokenField('checkout_step_1') !!}
                                    <fieldset>
                                        <div class="form-group">
                                            <label class="col-xs-4 col-xs-offset-4 control-label">Shipping: </label>
                                            <div class="col-xs-4">
                                                <select class="form-control" name="shipping_method">
                                                    @foreach(['mail'=>'Mail','collect'=>'Collect','express'=>'Express'] as $sMethod => $sMethodName)
                                                        <option value="{!! $sMethod !!}" {{ ($shippingMethod ?? '') == $sMethod ? 'selected' : '' }}>{!! $sMethodName !!}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                        ...
                        <tr class="info">
                            <td class="text-right" colspan="3">
                                <form class="form-horizontal" id="methods2">
                                    <fieldset>
                                        <div class="form-group">
                                            <label class="col-xs-4 col-xs-offset-4 control-label">Payment: </label>
                                            <div class="col-xs-4">
                                                <select class="form-control" name="payment_method">
                                                    @foreach(['wire transfer'=>'Wire Transfer','paypal'=>'Paypal','creditcard'=>'Credit Card'] as $pMethod => $pMethodName)
                                                        <option value="{!! $pMethod !!}" {{ ($paymentMethod ?? '') == $pMethod ? 'selected' : '' }}>{!! $pMethodName !!}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-xs-4">
                                                <input type="text" name="credit_card_number" id="creditCardField"
                                                       value="{!! $creditCardNumber ?? '' !!}" class="form-control"
                                                       required pattern="^[\d-]+$" />
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div class="col-xs-4 form-horizontal expires-fields">
                                                <select name="credit_card_month" id="creditCardMonthField" class="form-control" required>
                                                    @for($month = 1; $month <= 12; $month++)
                                                        <option value="{!! $month !!}" {{ $month == ($creditCardMonth ?? 0) ? 'selected="selected"' : '' }}>{!! sprintf("%02d", $month) !!}</option>
                                                    @endfor
                                                </select>
                                                <select name="credit_card_year" id="creditCardYearField" class="form-control" required>
                                                    @for($year = (int)date('Y'), $lastYear = $year + 10; $year <= $lastYear; $year++)
                                                        <option value="{!! $year !!}" {{ $year == ($creditCardYear ?? 0) ? 'selected="selected"' : '' }}>{!! $year !!}</option>
                                                    @endfor
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-xs-4">
                                                <input type="text" name="credit_card_cvv" id="creditCardCVVField"
                                                       value="{!! $creditCardCVV ?? '' !!}" class="form-control"
                                                       required data-bv-cvv="true" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <tr class="info">
                            <td class="text-right" colspan="3">
                                <div class="form-horizontal">
                                    <div class="form-group">
                                        ...
                                        <div class="col-xs-4">
                                            <input type="text" name="coupon" id="couponField" value="{!! $coupon ? $coupon->coupon : '' !!}" class="form-control" />
                                        </div>
                                    </div>
                                </div>
                            </td>
                            <td colspan="2" class="text-center">
                                ...
                                @include('cart._coupon_flash_widget')
                            </td>
                        </tr>
                        <tr class="info" id="currentCouponRow" style="{{ !$coupon ? 'display: none;' : '' }}">
                            <td class="text-right" colspan="3">
                                <label>Your coupon is &laquo;<span id="currentCoupon">{!! $coupon->coupon ?? '' !!}</span>&raquo;, and you have a discount:
                                    <span id="currentCouponDiscount">{{ $coupon->discount ?? 0 }}</span>%</label>
                            </td>
                            <td colspan="2" class="text-center">
                                <div class="form-horizontal">
                        ...
                        <tr class="danger">
                            ...
                            <th class="text-left" colspan="2">
                                ...
                                $ <span id="total_price">{{ $totalPrice }}</span>,-<br>
                            </th>
                        </tr>
                    </tfoot>
                </table>
        ...
        <div class="row">
            <div class="col-xs-8">
                ...
                <button class="btn btn-danger ladda-button" id="empty_cart" data-style="expand-right" data-spinner-color="#999999">
                    <span class="ladda-label"><span class="glyphicon glyphicon-trash"></span> Empty cart</span>
                </button>
            </div>
            <div class="col-xs-4">
