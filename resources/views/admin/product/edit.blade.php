                ...
                <form action="/admin/product-option-value/save" class="js-add-option-variant-form" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="field_optionID" name="optionID">
                            @foreach($allOptions ?? [] as $optId => $optName)
                                <option value="{{ $optId }}">{{ $optName }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="field_variantID">Variant</label>
                        <select class="form-control" id="field_variantID" name="variantID">
                        </select>
                    </div>
                    <div class="form-group">
    ...
    jQuery(function ($) {
        ...
        table.dataTable({
            ...
            columns: [
                ...
                { data: "price_surplus" },
                { data: "edit", orderable: false, searching: false },
                { data: "delete", orderable: false, searching: false }
            ]
        });
        ...
        showEditBlock = function (data) {
            ...
            var populate = { ID: '', price_surplus: 0 };
            if ('object' === typeof data && data) {
                $.extend(populate, data);
                populate.optionID = data.optionVariant___optionID;
            }
            $.each(populate, function (name, value) { $variantForm.find('[name="' + name + '"]').val(value); });
            $variantForm.data('bootstrapValidator').resetForm();
            if (populate.ID) {
                var id = parseInt(populate.variantID, 10);
                loadOptionVariants(populate.optionID, id).then(function () {
        ...
        removeVariant = function (id, confirmDeletion) {
            ...
            $.ajax({
                url: '/admin/product-option-value/delete', type: 'post', dataType: 'json',
                data: {id: id, confirm: +confirmDeletion}
            }).success(function (res) {
                if (res.error) { if (confirm(res.message)) { removeVariant(id, true); } }
                else { dataTable.ajax.reload(); }
            }).error(function () { alert('Error while deleting option'); });
        };
        ...
        loadOptionVariants = function (optionId, varToSelectId) {
            ...
            $.ajax({
                url: '/admin/option-value/get-option-values', type: 'post', dataType: 'json',
                data: {option_id: optionId},
                beforeSend: function () { $toDisable.attr('disabled', 'disabled'); },
                complete: function () { $toDisable.removeAttr('disabled'); }
            }).success(function (res) {
                var opts = res.optionVariants, optionsHtml;
                if (opts) {
                    optionsHtml = $.map(opts, function (name, varId) {
                        return '<option value="' + varId + '"' + (varToSelectId && varToSelectId == varId ? ' selected' : '') + '>' + name + '</option>';
                    }).join('');
                    $optionVariantsSelect.html(optionsHtml);
                }
                deferred.resolve();
            }).error(function () { deferred.reject(); alert('Error while loading variants'); });
            return deferred;
        };
        ...
        $optionVariantsPane.on('click', '.js-edit-variant', function (ev) {
            ev.preventDefault();
            var $link = $(ev.target), row = dataTable.row($link.closest('tr')).data();
        ...
        $optionVariantsPane.on('click', '.js-add-variant-button', function (ev) {
            ev.preventDefault();
            if ($variantEditBlock.is(':hidden')) { saveButtonText = 'Add option'; showEditBlock(); }
            else { hideEditBlock(); }
        });
        $variantForm.hzBootstrapValidator().on('success.form.bv', function(ev) {
            ...
            $.ajax({
                url: $variantForm.attr('action'), type: 'post', dataType: 'json',
                data: $variantForm.serialize()
            }).success(function (res) {
                if (res.error) { $variantFormErrors.html(res.message).show(); }
                else { dataTable.ajax.reload(); hideEditBlock(); }
            }).error(function (xhr, responseType, statusText) {
                if (statusText) { $variantFormErrors.html(statusText).show(); }
            }).complete(function () { $variantSaveButton.removeAttr('disabled'); });
        });
        $optionSelect.on('change', function () { loadOptionVariants($optionSelect.val()); });
    });
</script>
@endsection
