        ...
        <div class="col-xs-6 col-md-6 option-variants-pane js-option-variant-pane">
            ...
            <div class="variant-edit-block js-variant-edit-block">
                ...
                <form action="/admin/option-value/save" class="js-add-option-variant-form" method="post">
                    ...
                    <div class="form-group">
                        <label for="field_variant_sort_order">Sort Order</label><input type="text" class="form-control" required id="field_variant_sort_order" name="sort_order">
                    </div>
...
<script type="text/javascript">
    jQuery(function ($) {
        ...
        showEditBlock = function (data) {
            ...
            var populate = { variantID: '', name: '', sort_order: 0 };
            if ('object' === typeof data && data) { $.extend(populate, data); }
        ...
        removeVariant = function (id, confirmDeletion) {
            ...
            $.ajax({
                url: '/admin/option-value/delete', type: 'post', dataType: 'json',
            ...
            }).error(function () { alert('Error while deleting option variant'); });
        };
        ...
        $optionVariantsPane.on('click', '.js-delete-variant', function (ev) {
            ev.preventDefault();
            var $link = $(ev.target), row = dataTable.row($link.closest('tr')).data();
            removeVariant(row.variantID);
        });
        $optionVariantsPane.on('click', '.js-add-variant-button', function (ev) {
            ev.preventDefault();
            if ($variantEditBlock.is(':hidden')) { saveButtonText = 'Add variant'; showEditBlock(); }
            else { hideEditBlock(); }
        });
